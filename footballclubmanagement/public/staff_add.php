    ...
    if (empty($validationErrors)) {
        try {
            ...
            if (empty($employee_id)) {
                ...
                $stmt = $conn->prepare("SELECT MAX(CAST(SUBSTRING(employee_id, 4) AS UNSIGNED)) as max_id FROM staff WHERE employee_id LIKE ?");
                $stmt->execute([$dept_code . '%']);
                $result = $stmt->fetch();
                $next_id = ($result['max_id'] ?? 0) + 1;
                $employee_id = $dept_code . str_pad($next_id, 4, '0', STR_PAD_LEFT);
            }

            $stmt = $conn->prepare("INSERT INTO staff (name, position, department, phone, email, hire_date, salary, employee_id, qualification, experience_years, emergency_contact, emergency_phone, address, date_of_birth, gender, nationality, contract_type, contract_end_date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            if ($result) {
                $success = 'Staff member added successfully! Employee ID: ' . $employee_id;
...
<head>
    ...
    <title>Add Staff Member - Football Club Management</title>
...
<body class="bg-light">
            ...
            <div class="col-lg-10">
                <div class="card shadow-lg border-0">
                    ...
                    <div class="card-body p-4">
                        ...
                        <form method="POST" action="staff_add.php" id="staffForm" novalidate>
                            ...
                            <div class="form-section">
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select <?php echo isset($validationErrors['department']) ? 'is-invalid' : ''; ?>" 
                                                ...
                                                <option value="coaching" <?php echo ($_POST['department'] ?? '') === 'coaching' ? 'selected' : ''; ?>>Coaching Staff</option>
                                                <option value="medical" <?php echo ($_POST['department'] ?? '') === 'medical' ? 'selected' : ''; ?>>Medical & Sports Science</option>
                                                <option value="administrative" <?php echo ($_POST['department'] ?? '') === 'administrative' ? 'selected' : ''; ?>>Administration</option>
                                                <option value="technical" <?php echo ($_POST['department'] ?? '') === 'technical' ? 'selected' : ''; ?>>Technical & Analytics</option>
                                                <option value="operations" <?php echo ($_POST['department'] ?? '') === 'operations' ? 'selected' : ''; ?>>Operations</option>
                                                <option value="media" <?php echo ($_POST['department'] ?? '') === 'media' ? 'selected' : ''; ?>>Media & Communications</option>
                                                <option value="youth" <?php echo ($_POST['department'] ?? '') === 'youth' ? 'selected' : ''; ?>>Youth Development</option>
                                            </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="qualification" name="qualification" rows="2"
                                              placeholder="e.g., UEFA Pro License, Sports Science Degree, Medical Degree"><?php echo htmlspecialchars($_POST['qualification'] ?? ''); ?></textarea>
                                </div>
                            </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" id="contract_type" name="contract_type">
                                                ...
                                                <option value="permanent" <?php echo ($_POST['contract_type'] ?? '') === 'permanent' ? 'selected' : ''; ?>>Permanent</option>
                                                ...
                                                <option value="consultant" <?php echo ($_POST['contract_type'] ?? '') === 'consultant' ? 'selected' : ''; ?>>Consultant</option>
                                                <option value="internship" <?php echo ($_POST['contract_type'] ?? '') === 'internship' ? 'selected' : ''; ?>>Internship</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" id="status" name="status">
                                                ...
                                                <option value="on_leave" <?php echo ($_POST['status'] ?? '') === 'on_leave' ? 'selected' : ''; ?>>On Leave</option>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Employee ID format suggestion
            const departmentField = document.getElementById('department');
            const employeeIdField = document.getElementById('employee_id');
            if (departmentField && employeeIdField) {
                departmentField.addEventListener('change', function() {
                    if (this.value && !employeeIdField.value) {
                        ...
                        employeeIdField.placeholder = `e.g., ${deptCode}0001`;
                    }
                });
            }
